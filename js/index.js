...
var Status, calc, calc_game, calc_main, chart_default, draw, format_money, init, init_charts, init_st, logs_default, lot, play_game, push, reflect_games, reflect_main_game, set_game, start, stop;
...
calc_main = function() {
  ...
  return draw('all_game_main', window.logs.game_main.status_all_logs);
};
...
calc_game = function(id) {
  ...
  return draw('all_' + id, window.logs[id].status_all_logs);
};
...
draw = function(id, statuses) {
  if (statuses.length <= 0) {
    return;
  }
  window.charts[id].data.datasets[0].data = statuses.map(function(v) {
    return v.money;
  });
  window.charts[id].data.labels = statuses.map(function(v) {
    return v.step;
  });
  return window.charts[id].update();
};
